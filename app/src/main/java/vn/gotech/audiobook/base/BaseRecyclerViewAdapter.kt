...
//        val layoutResId = getChildLayoutResource(viewType)
        return createHolder(
            getChildLayoutResource(LayoutInflater.from(parent.context), viewType),
            viewType
        )
    }
    ...
    abstract fun getChildLayoutResource(layoutInflater: LayoutInflater, viewType: Int): ViewBinding

    abstract fun createHolder(v: ViewBinding, viewType: Int): ViewHolder<T>

    open class ViewHolder<T>(itemView: ViewBinding) : RecyclerView.ViewHolder(itemView.root) {
